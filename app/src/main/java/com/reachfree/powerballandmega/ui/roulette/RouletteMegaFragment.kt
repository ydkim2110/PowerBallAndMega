...
class RouletteMegaFragment : Fragment() {
    ...
    private val randomNumberMegaPlierList = ArrayList<Int>()
    private val selectedNumberList = ArrayList<Int>()
    private var sectors = Array(70) { i -> (i + 1).toString() }
    private var sectorsMegaPlier = Array(25) { i -> (i + 1).toString() }
    ...
    private fun spinPowerPlayWheel() {
        randomNumberMegaPlierList.clear()
        for (i in 1..25) {
            randomNumberMegaPlierList.add(i)
        }
        randomNumberMegaPlierList.shuffle()

        try {
            degree = DEGREE_BY_25 * randomNumberMegaPlierList[0]
    ...
    private fun calculatePowerPlayPoint(degree: Float) {
        ...
        do {
            if (degree in (initialPoint + 1f)..(endPoint + 1f)) {
                res = sectorsMegaPlier[i]
            }
            initialPoint += DEGREE_BY_25
            endPoint += DEGREE_BY_25
            i++
        } while (res == null && i < sectorsMegaPlier.size)
    ...
    private fun setDegreeAndDataToSelectedNumberList() {
        try {
            degree = DEGREE_BY_70 * randomNumberList[0]
    ...
    private fun calculatePoint(degree: Float) {
        ...
        do {
            ...
            initialPoint += DEGREE_BY_70
            endPoint += DEGREE_BY_70
            i++
    ...
    private fun inflateNumber(numberList: ArrayList<Int>, layoutContainer: LinearLayout) {
        ...
        for (i in numberList.indices) {
            ...
            if (i == 5) {
                lottoNum.setBackgroundResource(R.drawable.bg_mega_ball)
            }
    ...
    companion object {
        fun newInstance() = RouletteMegaFragment()
        private const val START_DEGREE = 0f
        private const val END_DEGREE = 720f
        private const val DEGREE_BY_70 = 360f/70f
        private const val DEGREE_BY_25 = 360f/25f
